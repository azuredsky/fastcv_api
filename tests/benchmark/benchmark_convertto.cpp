...
void benchmark(int inputWidth, int inputHeight){
    ...
    //warm up
    cvtColor<uchar, 3, uchar, 1, BGR2NV21>(u3_1, &u1_1);
    //BGR2NV21
    refrensh();
    show(convertto_u_1);
    show_(inputWidth);
    gettime((convertTo<uchar, 1, float, 1, 1>(u1_1, 1.0/255.0, &f1_1)));
    gettime_opencv((o_u1_1.convertTo(o_f1_1, CV_32F, 1.0/255.0)));

    show(convertto_u_3);
    show_(inputWidth);
    gettime((convertTo<uchar, 3, float, 3, 3>(u3_1, 1.0/255.0, &f3_1)));
    gettime_opencv((o_u3_1.convertTo(o_f3_1, CV_32FC3, 1.0/255.0)));

    show(convertto_u_4);
    show_(inputWidth);
    gettime((convertTo<uchar, 4, float, 4, 4>(u4_1, 1.0/255.0, &f4_1)));
    gettime_opencv((o_u4_1.convertTo(o_f4_1, CV_32FC4, 1.0/255.0)));

    refrensh();
    show(convertto_f_1);
    show_(inputWidth);
    gettime((convertTo<float, 1, uchar, 1, 1>(f1_1, 255., &u1_1)));
    gettime_opencv((o_f1_1.convertTo(o_u1_1, CV_8U, 255.)));

    show(convertto_f_3);
    show_(inputWidth);
    gettime((convertTo<float, 3, uchar, 3, 3>(f3_1, 255., &u3_1)));
    gettime_opencv((o_f3_1.convertTo(o_u3_1, CV_8UC3, 255.)));

    show(convertto_f_4);
    show_(inputWidth);
    gettime((convertTo<float, 4, uchar, 4, 4>(f4_1, 255., &u4_1)));
    gettime_opencv((o_f4_1.convertTo(o_u4_1, CV_8UC4, 255.)));
