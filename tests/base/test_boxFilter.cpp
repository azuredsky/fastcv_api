...
class BoxFilterTest : public testing::Test
{
    public:
        virtual void SetUp()
        {
            ...
            if(globalEnvironment::Instance()->testconfig.hasSection("BoxFilterTest"))
            {
                Cfg::cfg_type vec_arg = globalEnvironment::Instance()->testconfig.sectionConfigVec("BoxFilterTest");
...
TEST_F(BoxFilterTest, float_float){
    imageParam p ;
...
TEST_F(BoxFilterTest, uchar_uchar){
    imageParam p;
    ASSERT_TRUE(!imageparams.empty());
  //  for(int i = 0; i < imageparams.size(); i++)
 //   {
