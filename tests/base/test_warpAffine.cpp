...
class WarpAffineTest : public testing::Test
{
     public:
        virtual void SetUp()
        {
            ...
            if(globalEnvironment::Instance()->testconfig.hasSection("WarpAffineTest"))
            {
                Cfg::cfg_type vec_arg = globalEnvironment::Instance()->testconfig.sectionConfigVec("WarpAffineTest");
...
/*        size_t input_data_num = input_width*input_height*input_channels;
        ...
        Tdst* output_data1 = (Tdst*)malloc(output_height*dstep);
        ...
        //x86
        zeros<Tdst, ncDst>(&dst_x86);
        HPCStatus_t sta_x86 = warpAffine<Tsrc, ncSrc, ncDst, nc>(src_x86, map, type, &dst_x86);
...
TEST_F(WarpAffineTest, uchar_uchar_NEAREST_POINT){
        imageParam p ;
      //  ASSERT_TRUE(!imageparams.empty());
     //   for(int i =0; i<imageparams.size(); i++)
     //   {
                p = imagep;
              testWarpAffineFunc<uchar, 1, uchar, 1, 1, INTERPOLATION_TYPE_NEAREST_POINT, 1>(p, 1.01);
                testWarpAffineFunc<uchar, 3, uchar, 3, 3, INTERPOLATION_TYPE_NEAREST_POINT, 1>(p, 1.01);
                testWarpAffineFunc<uchar, 4, uchar, 4, 4, INTERPOLATION_TYPE_NEAREST_POINT, 1>(p, 1.01);
//}
        }
       

TEST_F(WarpAffineTest, uchar_uchar_LINEAR){
         imageParam p ;
     //   ASSERT_TRUE(!imageparams.empty());
    ///    for(int i =0; i<imageparams.size(); i++)
    //    {
                p = imagep;
              testWarpAffineFunc<uchar, 1, uchar, 1, 1, INTERPOLATION_TYPE_LINEAR, 1>(p, 1.01);
                testWarpAffineFunc<uchar, 3, uchar, 3, 3, INTERPOLATION_TYPE_LINEAR, 1>(p, 1.01);
                testWarpAffineFunc<uchar, 4, uchar, 4, 4, INTERPOLATION_TYPE_LINEAR, 1>(p, 1.01);
    //    }
       
}

TEST_F(WarpAffineTest, float_float_NEAREST_POINT){
        imageParam p ;
      //  ASSERT_TRUE(!imageparams.empty());
      //  for(int i =0; i<imageparams.size(); i++)
      //  {
                p = imagep;
               testWarpAffineFunc<float, 1, float, 1, 1, INTERPOLATION_TYPE_NEAREST_POINT, 255>(p, 1e-5);
...
TEST_F(WarpAffineTest, float_float_LINEAR){
        imageParam p ;
      //  ASSERT_TRUE(!imageparams.empty());
     ///   for(int i =0; i<imageparams.size(); i++)
      //  {
