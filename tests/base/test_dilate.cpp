...
class dilateTest : public testing::Test
{
     public:
        virtual void SetUp()
        {
            ...
            if(globalEnvironment::Instance()->testconfig.hasSection("dilateTest"))
            {
                Cfg::cfg_type vec_arg = globalEnvironment::Instance()->testconfig.sectionConfigVec("dilateTest");
...
int testDilateFunc(imageParam imagep,float diff_THR) {
    ...
    HPCStatus_t sta = dilate<T, ncSrc, ncDst, nc, EcoEnv_t::ECO_ENV_ARM>(src, element, &dst);
...
TEST_F(dilateTest, uchar_uchar){
    imageParam p;
  //  ASSERT_TRUE(!imageparams.empty());
   // for(int i = 0;i < imageparams.size();i++)
   // {
        p = imagep;
        testDilateFunc<uchar, 1, 1, 1, 1>(p,1e-5);
        testDilateFunc<uchar, 3, 3, 3, 1>(p,1e-5);
        testDilateFunc<uchar, 4, 4, 4, 1>(p,1e-5);
   // }
  
}

TEST_F(dilateTest, float_float){
    imageParam p;
  //  ASSERT_TRUE(!imageparams.empty());
 //   for(int i = 0;i < imageparams.size();i++)
  //  {
        p = imagep;
        testDilateFunc<float, 1, 1, 1, 1>(p,1e-5);
        testDilateFunc<float, 3, 3, 3, 1>(p,1e-5);
        testDilateFunc<float, 4, 4, 4, 1>(p,1e-5);
  //  }
